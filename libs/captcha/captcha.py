...
class Captcha(object):
    ...
    def initialize(self, width=200, height=75, color=None, text=None, fonts=None):
        ...
        # self.fonts = fonts if fonts else [os.path.join(self._dir, 'fonts', font) for font in ['Arial.ttf', 'Georgia.ttf', 'actionj.ttf']] 两外两种不理想
    ...
    def text(self, image, fonts, font_sizes=None, drawings=None, squeeze_factor=0.75, color=None):
        ...
        fonts = tuple([truetype(name, size)
                       for name in fonts
                       # for size in font_sizes or (65, 70, 75)])   (45, 50, 55)
                       for size in font_sizes or (55, 60, 65)])
        draw = Draw(image)
        char_images = []
        for c in self._text:
